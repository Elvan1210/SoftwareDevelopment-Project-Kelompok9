...
class _KelasManagementViewState extends State<KelasManagementView> {
  ...
  Future<void> _fetchInitialData() async {
    setState(() => _isLoading = true);
    // Kita hanya perlu fetch Kelas sekarang, data Guru & Siswa tidak perlu diload
    await _fetchKelas();
    if (mounted) setState(() => _isLoading = false);
  }
          ...
          return AlertDialog(
            ...
            content: SizedBox(
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    const SizedBox(height: 8),
                    // Keterangan informasi untuk Admin
                    Container(
            ...
            actions: [
              ...
              ElevatedButton(
                ...
                onPressed: () async {
                  if (namaCtrl.text.isEmpty) return; // Hanya mewajibkan nama kelas
                  
                  // PERBAIKAN: Menambahkan tipe Map<String, dynamic> secara eksplisit
                  ...
                  // Jika buat baru, siapkan array siswa_ids kosong (opsional)
                  if (!isEditing) {
                    body['siswa_ids'] = [];
                  }
  ...
  Widget build(BuildContext context) {
    ...
    return AppShell(
      child: Scaffold(
        ...
        body: _kelasList.isEmpty
            ? const EmptyState(icon: Icons.grid_view_rounded, message: 'Belum ada tim/kelas virtual.', color: Colors.blue)
            : RefreshIndicator(
                onRefresh: () async {
                  await _fetchKelas();
                  setState(() {});
                  ...
                  builder: (ctx, c) {
                    ...
                    return GridView.builder(
                      ...
                      gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                        ...
                        crossAxisSpacing: 16,
                        mainAxisSpacing: 16,
                        childAspectRatio: 1.5, // Sedikit diubah agar muat menampilkan kode akses
                      ),
                      itemCount: _kelasList.length,
                      itemBuilder: (context, index) {
                        final k = _kelasList[index];
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              Icon(Icons.vpn_key_rounded, size: 14, color: theme.primaryColor),
